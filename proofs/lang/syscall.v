...
Variant syscall_t : Type := 
  ...
  | ExternFun of string & seq atype & seq atype.
...
Definition syscall_sig_u (o : syscall_t) : syscall_sig_t := 
  ...
  | ExternFun _ args rets => {| scs_tin := args; scs_tout := rets |}
  end.
...
Definition syscall_sig_s {pd:PointerData} (o:syscall_t) : syscall_sig_t := 
  ...
  | ExternFun _ args rets => {| scs_tin := args; scs_tout := rets |} (* FIXME: esto no es correcto *)
  end.
